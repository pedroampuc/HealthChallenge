...
class PatientDAO
{
    var patientsData : [PFObject] = [PFObject]()
    var finish : Bool? = false
    func getPatientsData()
    {
        //
        //dispatch_async(dispatch_get_global_queue(Int(QOS_CLASS_USER_INITIATED.value), 0)) { // 1
            ...
            var query2 = PFQuery(className:"Paciente")
                ...
                } else
                {
                    ...
                    if let objects = objects as? [PFObject]
                    {
                        for object in objects
                        {
                            ...
                            self.patientsData.append(object)
                            println("id do objeto \(self.patientsData)")
                            i++
